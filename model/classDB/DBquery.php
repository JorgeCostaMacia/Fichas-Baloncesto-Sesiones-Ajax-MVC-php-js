...
class DBquery extends DBconnection {
    ...
    public function _prepareLogin($loginEmail, $loginPassword = ""){
        $arrayResut = [];
        if( $loginPassword != ""){
            $_ressult = $this->_connection->prepare('SELECT * FROM usuarios WHERE loginUsser = ? AND passUsser = ?');
            $ok = $_ressult->bind_param('ss',$loginEmail, $loginPassword);
            $ok = $_ressult->execute();
        }
        else {
            $_ressult = $this->_connection->prepare('SELECT * FROM usuarios WHERE loginUsser = ?');
    ...
    function _prepareUpdateStats($arrayStats){
        $_ressult = $this->_connection->prepare('UPDATE jugadores SET partidos = ?, puntos = ?, rebotes = ?, asistencias = ?  WHERE id = ?');
        ...
        $ok = $_ressult->bind_param('idddi',$partidos, $puntos, $rebotes, $asistencias, $id);

        foreach($arrayStats as $jugador){
            $id = $jugador->getId();
            $partidos = $jugador->getPartidos();
            $puntos = $jugador->getPuntos();
            $rebotes = $jugador->getRebotes();
